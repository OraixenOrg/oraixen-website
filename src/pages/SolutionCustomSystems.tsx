...
const ROUTE = '/solutions/custom-business-systems';
 ...
 *
 * "15+" is intentionally absent. Longevity, retention and international reach
 * are the stronger proof for the operations buyer this page addresses.
 */
...
export function SolutionCustomSystems() {
  const { t, i18n } = useTranslation('solutionCustomSystems');
  const market = marketFromLanguage(i18n.language);

  const problems = t('problems.items', { returnObjects: true }) as Item[];
  const buildItems = t('build.items', { returnObjects: true }) as Item[];
  const includes = t('includes.items', { returnObjects: true }) as Item[];
  const paths = t('modernization.paths', { returnObjects: true }) as Item[];
    ...
    <div className="bg-surface">
      ...
      >
        <Button
          ...
          size="lg"
          icon
          onClick={() => trackCtaClick('discuss_project', 'custom_systems_hero', '/contact')}
        >
          {t('hero.primaryCta')}
        </Button>
        <Button
          ...
          variant="outline"
          size="lg"
          onClick={() => trackCtaClick('view_process', 'custom_systems_hero', '/process')}
        >
          {t('hero.secondaryCta')}
        </Button>
      </PageHero>
      ...
      {/* ===================== 4. What the system can include ===================== */}
      <Section dark>
        <SectionHeading
          eyebrow={t('includes.eyebrow')}
          title={t('includes.title')}
          subtitle={t('includes.intro')}
        />
        <Stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {includes.map((item) => (
            <div
              key={item.title}
              className="rounded-2xl bg-card border border-line shadow-card p-5 h-full"
            >
      ...
      {/* ===================== 5. Modernization & integration ===================== */}
      <Section>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <Reveal>
            <Eyebrow className="mb-5">{t('modernization.eyebrow')}</Eyebrow>
            <h2 className="text-3xl md:text-4xl font-bold text-ink leading-tight">
              {t('modernization.title')}
            </h2>
            <p className="mt-5 text-lg text-body leading-relaxed">{t('modernization.p1')}</p>
            <p className="mt-4 text-lg text-body leading-relaxed">{t('modernization.p2')}</p>
          </Reveal>
      ...
      <Section dark>
        ...
        <Reveal className="mt-12 flex justify-center">
          <Button
            ...
            size="lg"
            icon
            onClick={() => trackCtaClick('discuss_project', 'custom_systems_mid', '/contact')}
          >
            {t('midCta.button')}
          </Button>
        </Reveal>
      ...
      <CTASection
        ...
        ctaLocation="custom_systems_final"
